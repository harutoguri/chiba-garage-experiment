...
/**
 * vehiclesDataから実ギャラリーの先頭動画bunnyVideoIdを取得するpure関数
 * vite.ts埋め込み・routers.tsキャッシュ・firstVideo API全てでこれを使う
 * bunnyStatus=readyフィルタなし（SDR変換済みならready不要）
 */
export function extractFirstVideoFromList(listData: any): { bunnyVideoId: string } | null {
  const vehicles = listData?.vehicles;
  const mediaMap = listData?.mediaMap;
  if (!vehicles?.length || !mediaMap) return null;
  for (const v of vehicles) {
    const media = mediaMap[String(v.id)] || mediaMap[v.id] || [];
    if (!media.length) continue;
    const sorted = [...media].sort((a: any, b: any) => (a.displayOrder || 0) - (b.displayOrder || 0));
    const firstVideo = sorted.find((m: any) => m.type === 'video' && m.bunnyVideoId);
    if (firstVideo?.bunnyVideoId) return { bunnyVideoId: firstVideo.bunnyVideoId };
  }
  return null;
}
